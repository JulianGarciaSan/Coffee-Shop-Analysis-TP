...
class FilterDuplicateNode:
    ...
    def is_duplicate(self, client_id: int, message_id: int) -> bool:
        ...
        """
        message_key = (client_id, message_id)
        
        if message_key in self.processed_messages:
            logger.info(f"Mensaje duplicado detectado: client_id={client_id}, message_id={message_id}")
            return True
        
        return False

    def mark_as_processed(self, client_id: int, message_id: int):
        """
        Marca un mensaje como procesado
        """
        message_key = (client_id, message_id)
        self.processed_messages.add(message_key)
        logger.debug(f"Mensaje marcado como procesado: client_id={client_id}, message_id={message_id}")
    ...
    def process_message(self, body: bytes, client_id: Optional[int], message_id: Optional[int]) -> bool:
        """
        Procesa un mensaje y lo reenvía si no es duplicado
        Returns True si se debe detener el consumo
        """
        ...
        try:
            ...
            if self.is_duplicate(client_id, message_id):
                logger.info(f"Mensaje duplicado ignorado: client_id={client_id}, message_id={message_id}")
                return False
            ...
            logger.debug(f"Mensaje reenviado: client_id={client_id}, message_id={message_id}")
            return False
    ...
    def _cleanup(self):
        ...
        logger.info(f"Estadísticas finales: {len(self.processed_messages)} mensajes únicos procesados")
